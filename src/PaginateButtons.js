...
export class PaginateButtons extends Component {
    getPageNumbers = () => {
    ...
    render() {
        ...
        return <React.Fragment>
            ...
            className="btn btn-primary mx-1">Previous</button>

            { current > 4 && <React.Fragment>
                <button className="btn btn-primary mx-1" onClick={ () => navigate(1) }>1</button>
                <span className="h4">...</span>
            </React.Fragment>
            }
            
            { this.getPageNumbers().map(num => 
                <button className={`btn mx-1 ${num === current} ? "btn-success" : "btn-primary"`}
            ...
            { current <= (pageCount - 4) &&
                <React.Fragment>
                    <span className="h4">...</span>
                    <button className="btn btn-primary mx-1" onClick={() => navigate(pageCount)}>{pageCount}</button>
                </React.Fragment>
            }

            <button onClick={ () => navigate(current + 1) } disabled={ current === pageCount }
             className="btn btn-primary mx-1">Next</button>
        </React.Fragment>
    }
}
